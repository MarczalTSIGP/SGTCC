...
data['examination_boards']
  ...
  if board_data['canceled'] == true
    ...
    puts "❌ Orientação cancelada para #{academic.name} - #{orientation.title} (#{board_type})"
  ...
  # --- Vincula atividades do calendário ---
  ...
  pdf_path = academic_pdf_dir.join('projeto.pdf')
  ...
  calendar.activities.each do |activity|
    ...
    # Filtra atividades cujo nome contenha “proposta”
    if activity.name.downcase.include?('projeto')
      aa.judgment = true
    ...
    if File.exist?(pdf_path)  
        ...
        document_type = "PROJETO" # Ex: Tipo de Atividade/Documento (Proposta, Versão Final)
        # --------------------------------
        ...
        puts "✅ Ciência registrada para atividade #{aa.activity_id} (Aluno: #{aa.academic_id})"
        aa.save!
    end
  end
end

puts "🎉 Importação de atas de PROJETO finalizada!"
