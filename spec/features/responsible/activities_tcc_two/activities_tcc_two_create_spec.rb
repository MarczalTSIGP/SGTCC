...
describe 'Activity::create', type: :feature do
  ...
  let(:calendar_tcc_two) { create(:calendar_tcc_two) }
  let!(:base_activity_types) { create_list(:base_activity_type, 3) }

  before do
    login_as(responsible, scope: :professor)
    visit new_responsible_calendar_activities_tcc_two_path(calendar_tcc_two)
  end
  ...
  describe '#create' do
    context 'when activity is valid', js: true do
      it 'create an activity with tcc 1' do
        ...
        expect(page).to have_current_path responsible_calendar_activities_tcc_two_index_path(
          calendar_tcc_two
        )
