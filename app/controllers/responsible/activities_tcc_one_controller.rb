class Responsible::ActivitiesTccOneController < Responsible::ActivitiesController
  def index
    add_breadcrumb I18n.t('breadcrumbs.tcc.one.index'), activity_url

    @activities = Activity.by_tcc(Activity.tccs[:one], @calendar)
  end
  ...
  def update
    if @activity.update(activity_params)
      ...
      redirect_to responsible_calendar_activities_tcc_one_path(@calendar, @activity)
    else
      flash.now[:error] = I18n.t('flash.actions.errors')
      render :edit
    end
  ...
  def activity_url
    responsible_calendar_activities_tcc_one_index_path(@calendar)
  end
end
